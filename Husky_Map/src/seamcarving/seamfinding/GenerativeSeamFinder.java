...
public class GenerativeSeamFinder implements SeamFinder {
    ...
    private static class PixelGraph implements Graph<Node> {
        ...
        public class Pixel implements Node {
            ...
            @Override
            public List<Edge<Node>> neighbors(Picture picture, EnergyFunction f) {
                List<Edge<Node>> neighb = new ArrayList<>(3);
                if (x + 1 == picture.width()) {
                    neighb.add(new Edge<>(this, sink, 0));
                } else {
                    for (int z = this.y - 1; z <= this.y + 1; z += 1) {
                        if (0 <= z && z < picture.height()) {
                            GenerativeSeamFinder.PixelGraph.Pixel to = new Pixel(this.x + 1, z);
                            neighb.add(new Edge<>(this, to, f.apply(picture, this.x + 1, z)));
                        }
                    }
                }
                return neighb;
